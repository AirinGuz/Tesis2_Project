...
clear all
%Mascaras
ExcelNameFile='Lotes_2021.xlsx';
img="20201013_143255_48_2277_3B_AnalyticMS_SR_8b_clip.tif";
info = imfinfo(img);
dateTaken = info.DateTime;
[L1,L2,L3,L4,L5,L6,L7]=ImgProc(ExcelNameFile, img);
L1_img=ShowImage(L1);
L2_img=ShowImage(L2);
L3_img=ShowImage(L3);
L4_img=ShowImage(L4);
...
function [L1,L2,L3,L4,L5,L6,L7]=ImgProc(ExcelNameFile,ImgFileName)

    [imgdata,R] = readgeoraster(ImgFileName);
    
    L1_matrix = readmatrix(ExcelNameFile,'Sheet','043','Range','B2:BD178');
    %L2 = readmatrix('Lotes_2021.xlsx','Sheet','386 A','Range','B2:AZ104');
    %L3 = readmatrix('Lotes_2021.xlsx','Sheet','386 B','Range','B2:BK154');
    L2_matrix = readmatrix(ExcelNameFile,'Sheet','387','Range','B2:BE198');
    L3_matrix = readmatrix(ExcelNameFile,'Sheet','389A','Range','B2:Y160');
    L4_matrix = readmatrix(ExcelNameFile,'Sheet','389 B','Range','B2:AL137');
    L5_matrix = readmatrix(ExcelNameFile,'Sheet','395','Range','B2:BS160');
    L6_matrix = readmatrix(ExcelNameFile,'Sheet','399A','Range','B2:AK135');
    L7_matrix = readmatrix(ExcelNameFile,'Sheet','399B','Range','B2:AI173');
